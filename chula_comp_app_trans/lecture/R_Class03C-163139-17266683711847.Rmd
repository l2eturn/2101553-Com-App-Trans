...
R has an abbreviated name for every probability distribution. This name is used to identify the functions associated with the distribution. For example, the name of the Normal distribution is “norm,” which is the root of the function names listed in **Table 1**.
...
There are random number generators for all built-in distributions. Simply prefix the distribution name with “r” and you have the name of the corresponding random number generator.

```{r Random numbers, warning=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=80)}

# Generate a vector of 10 numbers between 0 and 1

# One uniform variate between -3 and +3

# One normal variate, mean 100 and SD 15

# One binomial variate
...
This is a powerful capability in cases such as hierarchical models, where the parameters are themselves random. The next example calculates 30 draws of a normal variate whose mean is itself randomly distributed and with hyperparameters of *μ* = 0 and *σ* = 0.2:
...
# Generates a sequence of 20 Bernoulli trials—random successes or failures.
...
which means “select all the elements of `v` in random order while using each element exactly once.” That is a random permutation.

Here is a random permutation of 1, …, 10:
...
For a cumulative probability, *P*(*X* ≤ *x*), use the distribution function. All built-in probability distributions have a distribution function whose name is “p” prefixed to the distribution name; thus, `pbinom` is the distribution function for the binomial distribution.
...
The cumulative probability, *P*(*X* ≤ *x*), is given by the *distribution function*, which is sometimes called the *cumulative probability function*. The distribution function for the binomial distribution is `pbinom`. Here is the cumulative probability for *x* = 7 (i.e., *P*(*X* ≤ 7)):
...
The *interval probability*, *P*(*x*~1~ \< *X* ≤ *x*~2~), is the probability of observing *X* between the limits *x*~1~ and *x*~2~. It is calculated as the difference between two cumulative probabilities: *P*(*X* ≤ *x*~2~) – *P*(*X* ≤ *x*~1~). Here is *P*(3 \< *X* ≤ 7) for our binomial variable:
...
Use the distribution function, which calculates *P*(*X* ≤ *x*). All built-in probability distributions have a distribution function whose name is “p” prefixed to the distribution’s abbreviated name—for instance, `pnorm` for the normal distribution.
...
Remeber that the continuous variables have no “probability” at a single point, *P*(*X* = *x*). Table 5 gives the distribution functions for several continuous distributions.
...
Also like discrete probabilities, the interval probability for a continuous variable, *P*(*x*~1~ \< *X* \< *x*~2~), is computed as the difference between two cumulative probabilities, *P*(*X* \< *x*~2~) – *P*(*X* \< *x*~1~). For the same exponential variable, here is *P*(20 \< *X* \< 50), the probability that it could fall between 20 and 50:
